...
| GET | `/v1/demos/{id}/airgibs` | `preMs` | `{timeUnit, preMs, airgibs: []result.AirgibEvent}` (Key Moments: direct rocket hits on airborne victims, height-sorted; empty without the map BSP). `preMs` is the pre-hit look-back (default 200, range `0..1000`): the victim must be airborne at the hit AND `preMs` before it, since the damage entry lands after the rocket's own knockback moved them. `preMs=0` drops the pre-hit gate (the pre-v71 rule); the default serves the stored list, any other value recomputes, and the effective value is echoed |
