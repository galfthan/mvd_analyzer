...
func TestPortalOffByDefault(t *testing.T) {
	...
	srv := httptest.NewServer(newRouter(&fakeStore{}, logger, "", testUploadConfig, nil, nil, &fakeSearcher{}))
	defer srv.Close()
...
func TestPortalExemptFromAPIKey(t *testing.T) {
	...
	srv := httptest.NewServer(newRouter(&fakeStore{}, logger, "", testUploadConfig, auth, p, &fakeSearcher{}))
	defer srv.Close()
