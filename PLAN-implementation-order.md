...
| 12 | Always-full API parse (decided 2026-07-07, measured: +3-4% parse, +5% cache): mvd-api builds projectiles/beams/nails at cold parse; EnsureShotStreams + the shot-streams lazy artifact + degrade path retired (the F12 class becomes impossible; first /shots stops costing a second full parse). WASM gains BuildNails (ng/sng aim stats + nail overlay). los stays the one genuinely lazy artifact. |
