...
| 15 | **NEXT** — Discord-auth key portal (`internal/portal`, OAuth2 identify, one key per user) + the phase-14 carry-forward store cross-process lock. Automated gate uses a stubbed Discord (httptest); the live-Discord gate is deferred to operator provisioning. [PLAN-hosting.md](PLAN-hosting.md) |
