...
func TestUpdateStat_HealthTransitionsEmitDeathAndSpawn(t *testing.T) {
	...
	if err := p.updateStat(0, mvd.StatHealth, -60, 10030); err != nil {
		t.Fatalf("gib update: %v", err)
	}
	// Instant respawn: -60 → 100 should emit StatUpdateEvent then SpawnEvent.
	if err := p.updateStat(0, mvd.StatHealth, 100, 10040); err != nil {
	...
	if d.PlayerNum != 0 || d.TimeMs != 10030 {
	...
	if s.PlayerNum != 0 || s.TimeMs != 10040 {
...
func TestUpdateStat_HealthNoTransitionDoesNotEmitDeathOrSpawn(t *testing.T) {
	...
	if err := p.updateStat(0, mvd.StatHealth, 50, 1000); err != nil {
...
func TestUpdateStat_FirstHealthFiresSpawn(t *testing.T) {
	...
	if err := p.updateStat(0, mvd.StatHealth, 100, 500); err != nil {
		t.Fatalf("first update: %v", err)
	}
	if err := p.updateStat(0, mvd.StatHealth, 100, 600); err != nil {
