...
| `matchStartUnannounced` | `statusAtOpen` is not running, `status` became running DURING the recording, and none of the four Layer-1 start signals produced an analyzable match | 138 → **0** (v75) | The server started a match under our watch and none of the signals this pipeline reads reached it. **Schema v75 emptied this bucket.** Re-running all 138 demos with the three non-print start signals in place, every one of them now detects a start and produces a full result: 104 on the `matchdate:` stamp (the KTX matchless FFA servers, which never print a start line — `ktx/src/match.c:1294-1297`) and 34 on the `status` transition alone (the ktx 1.38 / 1.40-beta demos plus all 24 `fortress` and 8 `ctf` demos, whose mods write their own running clock into the key). The reason is retained: a server can still move `status` to a running clock and yield no player stream at all. |
