...
view (schema v40). They are **opt-in** — built only when requested
(`qw-analyze -include projectiles,beams`; the WASM map build always builds
them since the result stays in browser memory) — because they are sizeable
(thousands of beams in a team game). Both are columnar (parallel arrays, one
entry per flight / bolt), times match-relative ms.
...
| Version | Changes |
|---|---|
| v40 | `Streams` gains two opt-in spatial weapon-fire streams for the map view: `streams.projectiles` (`ProjectileStreams` — every tracked rocket/grenade flight as a spawn→despawn segment + times) and `streams.beams` (`BeamStreams` — every LG `TE_LIGHTNING2` bolt as a muzzle→impact segment + time), both columnar. Built only when requested (`qw-analyze -include projectiles,beams`; the WASM map build) so the default output and golden corpus stay lean. Additive (`omitempty`); absent from the default parse. |
