...
perspective). Both ids come from `playerUserIDs`, so they are the
last-with-play id of that name — see
[Player userids](#player-userids-schema-v66).
...
| `airgibs[].attackerUserID` / `.victimUserID` | via `playerUserIDs`, i.e. the last-with-play id |
...
- **A name-keyed id is the LAST one, and it is a choice.** Where the
  ...
  reports the id of the last session that had play — the one that is live
  at the end of the demo and the one a `track=` resolves for a player still
  connected. The earlier ids of the same person are not published here; the
...
- **It partitions rows.** A theoretical clash (a userid reissued to the same
  ...
  suffix, because the streams builder groups on this key and a clash would
  splice two players into one row.
...
**player name**, which is the same canonical string in all of them; fetch
`/player-stats` once and index it by name to get the identity and sessions
for any of them.
...
| EndMs | `endMs` | int32 ms | The drop broadcast, or the next connection's userinfo. Half-open `[startMs, endMs)`. The one **synthetic** bound: a client still connected when the recording ended has no wire event to report, so it is closed at match end. |
| Slot | `slot` | int | Wire client slot (0-based) — the index `svc_*` messages address this connection by. |
| UserID | `userId` | int | The connection's userid: the value a hub `?track=` link must carry to follow this player **inside this window, and only then**. |
...
no userid cannot answer the question the list exists for. A row can therefore
carry an `identity` with **no** `sessions` (every occupancy unidentified),
which is a different statement from carrying no identity at all.
