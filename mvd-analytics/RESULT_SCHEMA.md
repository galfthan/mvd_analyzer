...
world), and the victim's height is ≥ 96 units (≈ two player models) at
**two** samples: the one nearest the hit **and** the one nearest
`hit − preMs` (default 200 ms; schema v71). The pre-hit gate is not a
smoothing filter — it removes a class of provably wrong entries. KTX stamps
the damage entry at (or one wire frame after) the physics frame in which the
rocket's own knockback has already moved the victim, so the hit-time sample
alone reports players who were STANDING at impact as airborne (measured: a
victim riding the dm2 `func_train` read 303 units of air when a quad direct
rocket blasted him off it). A plain jump peaks at ~45 units, so a genuine
96+ victim has been above the threshold for hundreds of ms and the look-back
cannot drop a real event. Both samples must also be within 250 ms of the
instant they answer for, and neither may be `NoFloor`. Reported `height`,
`loc` and `heightAboveAttacker` always come from the **hit** sample — the
pre-hit sample only gates.
...
(`preMs=0` = no pre-hit gate, the pre-v71 rule), echoing the effective value
