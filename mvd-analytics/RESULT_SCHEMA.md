...
**0.73%** aggregate error / 45.9% of rows exact on the 567 `fixed` rows,
13.0% / 31.2% on the 16 `spread` ones and 22.2% / 61.8% on the 55
`unestablished` ones (1.34% pooled over 638): `spread` — a demo whose own
...
demos: `rl` **46.9% row-exact / 1.34% aggregate** (bias −0.14 per row,
...
| `rl` | 99.8% exact | comparable **since v74**, because the row publishes KTX's own DIRECT-impact convention there (`recon.directHits`): **1.34% aggregate**, 46.9% of 638 rows exact, p90 error 2 hits. The any-path count it used to publish read ~4x higher (361% aggregate, 35 pp per row) |
