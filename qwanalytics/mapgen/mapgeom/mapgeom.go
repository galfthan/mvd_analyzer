...
// MapItem is a single pickup item present on the map: type, world
// origin, and (for item_health) the "size" distinction between H15,
// H25, and MH that the Quake progs encode as spawnflags.
type MapItem struct {
...
// MapItemsFile is the slim items-only JSON per map, generated by
// mapgen alongside the geometry JSON and embedded by the items
// package so the analyzer has a compact per-map item corpus.
type MapItemsFile struct {
	Map     string    `json:"map"`
	Version int       `json:"version"`
	Items   []MapItem `json:"items"`
}

// EntityToItemKind maps a Quake classname (+ spawnflags for
// item_health) to the compact item Kind string used in MapRegions.
// Returns "" for entities we don't track as pickups. The kinds match
// the analyzer / result schema naming used elsewhere in the project
// (ga, ya, ra, h15, h25, mh, rl, lg, ssg, ng, sng, gl, shells, nails,
// rockets, cells, quad, pent, ring).
func EntityToItemKind(classname string, spawnflags int) string {
	switch classname {
	case "item_armor1":
		return "ga"
	case "item_armor2":
	...
	case "item_armorInv":
		return "ra"
	case "item_health":
		// Quake item_health spawnflags: SUSPENDED=1 is about
		// positioning, not size; SPAWNFLAG_SUPERHEALTH=2 is the
		// megahealth flag; bit 0 (ROTTEN) marks the 15 HP variant.
		// Test MH first because its flag bit (2) can coexist with
		// bit 0 in weird custom maps, though standard Quake doesn't
		// combine them.
		if spawnflags&2 != 0 {
			return "mh"
		}
