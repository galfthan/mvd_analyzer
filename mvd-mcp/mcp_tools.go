...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getLocGraph",
		Description: "Per-map adjacency graph of named locations: which locs are reachable from which, with edge weights derived from per-player loc-to-loc transitions. Useful for movement-pattern reasoning ('what's adjacent to RA?'). Node weights (total/byPlayer/byTeam and the armed/unarmed/quad/pent breakdowns) are ALIVE time-spent values in integer milliseconds (pure-ms model) — time while dead is excluded (schema v64), so these are lower than wall time in the loc; edge weights are transition counts, likewise excluding a corpse's travels. The response echoes ms in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getLocTrails",
		Description: "Per-player sequence of loc residences with dwell durations. Residences are ALIVE dwell (schema v64): a residence is truncated at a death and resumes at the respawn, and the final one ends at the player's last position sample rather than at match end, so it agrees with getLocGraph node time. minDwellMs filters nearest-loc flicker (MCP default 250; pass 0 for raw). Each residence is a resolved loc name by default; pass loc='index' for raw LocTable indices (decode via getLocTable). Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
