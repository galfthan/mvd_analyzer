...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getLocGraph",
		Description: "Per-map adjacency graph of named locations: which locs are reachable from which, with edge weights derived from per-player loc-to-loc transitions. Useful for movement-pattern reasoning ('what's adjacent to RA?'). Node weights (total/byPlayer/byTeam and the armed/unarmed/quad/pent breakdowns) are time-spent values in integer milliseconds (pure-ms model); edge weights are transition counts. The response echoes ms in timeUnit.",
