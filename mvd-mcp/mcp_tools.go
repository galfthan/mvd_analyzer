...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getFrags",
		Description: "Frag aggregates + full kill log. totalFrags + byPlayer (kills/deaths/byWeapon per player) + byWeapon (kills per weapon) + frags (every kill with t (match-relative seconds)/killer/victim/weapon/isSuicide/isTeamKill). NOTE: unlike getDamage/getAim/getItems, summary defaults FALSE here — the kill log is small (one row per frag) and usually the point; pass summary:true to drop it. Optional players= / weapons= filters narrow both aggregates and log. Empty-log convention: frags is null when dropped by summary=true, [] when included but the filter matched nothing. Use this instead of aggregating getEvents(types:['frag']) yourself.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getChat",
		Description: "All-chat and team-chat messages within an optional time window. Returns {messages:[...]} where each message has t (match-relative seconds), type ('chat' or 'teamsay'), player, team, message (raw with ezQuake markup), messageClean (markup stripped). Cheaper and shape-cleaner than getEvents(types:['chat']) when you only want chat.",
