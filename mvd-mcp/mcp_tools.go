...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getStateAt",
		Description: "Point-in-time state per player at a given match-relative time. Carry-forward for change streams; nearest-sample for position; interval membership for held items/powerups. Times are seconds by default; pass units:'ms' for int milliseconds (timeUnit echoes the effective unit).",
	...
	addTool(s, &mcp.Tool{
		Name:        "getLocTrails",
		Description: "Per-player sequence of loc residences with dwell durations. minDwellMs filters nearest-loc flicker (MCP default 250; pass 0 for raw). Each residence is a resolved loc name by default; pass loc='index' for raw LocTable indices (decode via getLocTable). Times are seconds by default; pass units:'ms' for int milliseconds (timeUnit echoes the effective unit).",
