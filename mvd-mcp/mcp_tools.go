...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getOverview",
		Description: "Return a curated summary of the demo (map, teams, players) plus the per-demo CAPABILITY MANIFEST. `map` is the canonical shortname (dm2, dm3, …; joinable with searchGames rows and getMetadata serverinfo); `mapTitle` is the display-only level title (e.g. \"Claustrophobopolis\"), present only when it differs from `map` — never use it as an identifier. Who won: `teams` and `players` are sorted by frags descending, so teams[0] (team modes) / players[0] (duel/FFA) is the winner — frags is the canonical net score. WHEN WAS IT PLAYED: `timing.matchStartUnixMs` (schema v72) is the server's wall clock at match-relative t=0 — the field to answer that with, NOT `demoStartUnixMs`, because it is what the wire date markers (the KTX `matchdate` / kmod `matchkey` broadcasts, the ktxstats date, the `//finalscores` stamp) actually state, and it reaches ~95% of the archive against ~25% for the demo-open server clock. ALWAYS read `timing.matchStartConfidence` beside it: `exact`, `unverified` (nothing contradicts it but something is unpinned — usually a stamp that printed no timezone), or `contradicted` (a check the value provably fails, e.g. a date predating the server binary's own release — old servers ran with unset clocks). A contradicted anchor is REPORTED, never dropped or silently corrected, and `timing.matchStartNote` names the failed check; `timing.matchStartSource` names where the value came from (`mvdhidden`/`epoch` server clock, or `matchdate`/`matchkey`/`ktxstats`) and `timing.matchStartAccuracyMs` is its ± window, which runs from 1 ms up to 50400000 (±14 h) when the stamp carried no zone at all — so never present a marker-derived time to a user without its accuracy and grade. `timing.matchEndUnixMs` is the match-end stamp where the demo carried one. Two degradation signals, deliberately separate: `errors` is the ANALYZER's non-fatal errors over events that decoded fine, while `parseWarnings` (schema v72) is the READER's census of wire bytes that never decoded at all (unknown svc_* commands, unknown temp-entity or hidden-message types, payloads that failed to parse — with exact `total`, per-category `byType` and a capped sample table). `parseWarnings` is omitted on a clean parse, which is the normal case; either being non-empty means the result is degraded and some sections may be missing or thin, so check both before trusting detail views. Also note `mode`: since v72 it falls back to KTX's own mode name from the demoinfo block or the `//finalscores` stuffcmd where the demo had no countdown, so it is filled on the pre-ktxstats half of the archive too — the two vocabularies spell differently (\"Duel\" vs \"duel\"), so compare case-insensitively and read /metadata for the exact source. Use this first to decide which detailed view to query next, and BRANCH ON `available` instead of spending a call to find out: it carries one flag per detailed view (demoInfo, metadata, frags, damage, shots, aim, locGraph, opening, playerStats, regionControl, height, liquid, los), each mirroring the predicate behind that view's 422, so a false is exactly the 422 you would have got. height/liquid/los are the ones you CANNOT infer any other way — they depend on which map BSPs the server was provisioned with, not on what the demo recorded, so the same demo can answer differently on two deployments; `los` covers BOTH the los and pvs sets (one pass, one gate, pvs ⊇ los) and is a prediction rather than a reading because that pass is heavy and lazy. A true flag means the section EXISTS, not that it is non-empty. This response no longer inlines highlight lists (topKills/topStreaks/topPowerups were removed in v70 as copies of getTopKills, getLives and getEvents) — fetch those tools when you want the rows. UNITS: overview times (duration, matchStart/End) and every tool's startTime/endTime/time input are ALL integer MILLISECONDS (pure-ms model) — an overview time carries straight into a filter with no conversion. Response shape: see mvd-api /v1/demos/{id}/overview. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getDemoInfo",
		Description: "KTX demoinfo blob — per-player weapon accuracy (hits/fires), kills/deaths/TK, damage dealt/taken, sprees, control time, item pickup counts, RL/LG transfers. Authoritative KTX scoreboard, VERBATIM: the server's own counters, unaltered. It 422s on 54% of the archive — the whole pre-ktxstats half, which no reprocessing will ever fix — so never branch on this tool alone; getPlayerStats answers the same questions on every demo and is the one to fall back to (and getOverview's `available.demoInfo` says which you have without spending a call). WHAT THE NUMBERS COUNT differs per weapon, and the difference is large enough to fabricate trends: `acc.rl.hits` / `acc.gl.hits` count only projectiles that TOUCHED a player (ktx/src/weapons.c:994, :1329), while `acc.sg.hits` and `acc.sg.attacks` are both PELLET counts (6x / 14x a trigger-pull count for sg / ssg). getPlayerStats' derived accuracy counts a fire that landed damage by ANY path and trigger pulls, which reads ~4x higher on rl and ~1.5x on gl than the block does — so do NOT mix rows from the two tools into one series. getPlayerStats publishes the machine-readable marker for exactly this: `accuracy.byWeapon[].hitsConvention` (anyDamage | directImpact | pellets); gate any cross-demo aggregation on it.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getMetadata",
		Description: "Server cvars + KTX match settings: mode, timelimit, fraglimit, antilag, spawnmodel (k_spw), midair, instagib, overtime, powerups, noitems, vwep, noweapon, matchtag. Plus the full fullserverinfo cvar dump (hostname, version, watervis, dmgfrags, etc.). Used to answer 'what ruleset was this played under'. Also carries `finalScores` (schema v72): KTX's `//finalscores` end-of-match scoreline verbatim — {date, mode, map, team1, score1, team2, score2}. It rides 64% of the archive against the KTX demoinfo block's 46%, so on the pre-ktxstats half it is the only place the SERVER states a final result, a mode and a map; scores can be negative (suicides), `mode` is KTX's lastscores spelling ('duel', 'team', 'FFA', 'Clan Arena'), and `date` is strftime '%b %d, %H:%M' with NO year, no seconds and no timezone — do not read it as an instant, use getOverview's `timing` block for the resolved wall clock.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getPlayerStats",
		Description: "Canonical per-player and per-team statistics — the one-call scoreboard. Corrected frags + deaths always; kills/suicides/teamKills/efficiency/byWeapon/maxSpree/maxQuadSpree are OMITTED TOGETHER (render '-', not 0) on a demo whose obituary frag log measured nothing while players died. score.maxSpree (longest run of kills between deaths) and score.maxQuadSpree (longest run of kills made while holding the quad — resets on death AND on a fresh quad pickup, mirroring KTX items.c:2180) are the derived equivalent of the KTX demoinfo block's spree.max / spree.quad, which 54% of the archive has no block to carry: they are ALWAYS derived, never overlaid from KTX, so a 0 inside a present family is an observed zero, and a TEAM row carries the BEST any member ran, never a sum. One deliberate divergence from KTX: its increment gate is `strneq(attackerteam, targteam) || !tp_num()` (ktx/src/client.c:4865), so wherever teamplay is OFF — every duel, every FFA — a player's own SUICIDE bumps their streak in the same call that latches it; ours counts only the kills score.kills counts, so a duel with self-kills reads exactly 1 lower per affected streak. Withheld-and-compared against the verbatim KTX block on 188 archive demos / 665 rows: maxQuadSpree 99.8% exact, maxSpree 92.6% overall and 99.6% on rows where kills already agrees with KTX and the player never suicided. Damage — including damage.byWeapon / byWeaponTeam / byWeaponSelf, the given/givenTeam/givenSelf splits by attacker weapon, where byWeapon+byWeaponTeam are measured whenever the family is present (KTX writes both counters in one sub-block) while byWeaponSelf is measured only where a damage stream was read, i.e. exactly when damage.taken is present — per-weapon accuracy, per-kind pickup tallies, and POSSESSION TIME: how long each player held each weapon and armor type (the armor family is omitted entirely when the demo carries no armor stream; the no-armor complement is the hold.armor.none field). Available on EVERY demo, including old ones with no KTX demoinfo block. Every stat family carries src: 'derived' | 'ktx', plus 'derived:unbounded' on damage (raw wire values incl. overkill — never compare with bounded rows) and 'reconstructed' on damage when the demo predates the KTX damage instrumentation and the family rides the damage reconstruction (~1% match-total estimates; on such demos the enemyVs*/ewep buckets read 0 when the recording froze its weapon bits — unmeasurable, not zero, and getDamage's coverage field — NOT restated here — says how much of the match the reconstruction actually saw: READ IT BEFORE QUOTING A RECONSTRUCTED DAMAGE ROW AS A MATCH TOTAL, because that ~1% assumes the recording carried the evidence, and on a low-coverage demo every reconstructed damage figure on the row is a FLOOR, not an estimate) and 'mixed' on a team row whose members disagree (a data-integrity canary; player rows never carry it). ACCURACY takes 'reconstructed' too (its enum, and the sources roll-up's, is derived|ktx|reconstructed|mixed): on a demo whose damage source is reconstructed (no wire dmgdone stream) accuracy.byWeapon[].hits is FILLED from the published aim recon tier — the same counts getAim publishes as weapons[].recon.hits — instead of being withheld, and only the weapons that tier validated carry a number (lg, sg, ssg, axe, rl, gl) while ng/sng keep hits ABSENT, which means 'not recovered for this weapon', never 'no hits'; a family whose weapons all fall outside the tier stays 'derived', attacks being shot-derived either way. Those hits inherit the same completeness caveat as the damage family they are built from: on a demo whose getDamage coverage is low they are a FLOOR, not an estimate — the tier can only count what the reconstruction saw — so read coverage before deriving an accuracy from them. Grade before diffing against KTX: derived lg hits agree to 0.9% in aggregate, but rl/gl do NOT — KTX's rl/gl hits is the DIRECT-impact count (ktx/src/weapons.c:994, :1329) while ours counts a fire that landed damage by any path, so ours reads ~4x higher on rl and ~1.5x on gl, and sg/ssg are pellets on KTX's side against trigger pulls on ours; attacks matches KTX to the row on every single-projectile weapon. DO NOT re-derive that rule: every weapon carrying hits also carries accuracy.byWeapon[].hitsConvention — 'anyDamage' (a fire that landed damage by any path; every derived/reconstructed weapon and KTX's lg/axe/ng/sng), 'directImpact' (KTX's rl/gl only) or 'pellets' (KTX's sg/ssg, where attacks counts pellets too). Two rows are comparable exactly when weapon AND hitsConvention match, so gate any cross-demo or cross-era aggregation on it rather than on src, which states the evidence grade and nothing about what is being counted. It is per weapon because one src:'ktx' row uses all three at once. The KTX convention cannot be recovered on a demo with no block: the wire damage log's splash flag reproduces KTX's rl count exactly (638/638 rows) but the pre-instrumentation half carries no such flag, and the reconstruction's geometric direct/splash verdict answers it for gl (1.2% aggregate) and not for rl (+80%), so archive rl hits stay 'anyDamage' by measurement, not by omission. score.byEnemyWeapon and damage.byEnemyWeapon are the VICTIM-weapon axis — the same kills and the same enemy damage split by what the TARGET was holding, the complement of the byWeapon maps which key on the ATTACKER's weapon. This is weapon denial: 'how many armed enemies did this player take out'. Buckets are MUTUALLY EXCLUSIVE: both (victim held RL and LG), rl (RL not LG), lg (LG not RL), mid (ssg/sng/gl), sg (nothing above shotgun tier), plus unknown on the kill side. score.byEnemyWeapon sums to score.kills EXACTLY (score is never overlaid); damage.byEnemyWeapon sums to the enemy damage this pipeline reconstructed, which equals damage.given on a src:derived row but NOT on a src:ktx one (there given is KTX's own counter and the split stays ours — a 0.03% residual over the corpus), so do not expect a share-of-given from these buckets to reach exactly 100% on a KTX row. CRITICAL: enemies killed while holding an RL is rl + both, NEVER rl alone — the same for lg — and adding the two double-counts the players in both. Derived on every demo with streams (KTX's own ekills is inclusive and mode-suppressed, so it is never overlaid); score.byEnemyWeapon is absent exactly when kills is, while damage.byEnemyWeapon needs the damage stream and is present exactly when damage.taken is. damage.enemyWeapons stays the lg+rl+both summary. score.byWeaponVsEnemyWeapon is the JOINT distribution those two kill maps are marginals of (killer weapon -> victim bucket -> kills) — use it for what neither marginal answers, e.g. 'how many of this player's LG kills were against enemies carrying an RL'; summing it reproduces byWeapon (over inner keys) and byEnemyWeapon (over outer keys) exactly. getDemoInfo remains the verbatim KTX block to diff against. Each PLAYER row also carries identity + sessions: identity is the reconnect-unification key — two rows with the SAME value are one human under two names (a player who reconnects while their old connection is still spawned is renamed `(N)name` by the server and scored as two players, which this pipeline reproduces rather than merging), and it is DEMO-LOCAL, so never persist it or compare it across demos (the cross-demo identity is login). sessions[] lists every wire occupancy behind the row in time order — {startMs, endMs, slot, userId, name} — so a hub ?track= link can use the userId VALID AT THAT INSTANT instead of the single per-player id in getOverview. Both are absent on team rows and on a scoreboard-only player (no stream). Other name-keyed tools (getLives, getTopWindows, getFrags, getDamage, getBuckets) join to these rows by player NAME — except when two identities share a display name, where this tool suffixes both rows `name#slot` while the frag and damage logs keep the bare name: strip the `#…` suffix to join, and read the answer as covering both same-named players. Hold times are exact integrals with explicit denominators (window.matchMs / presentMs / aliveMs) — read shareAlive/shareMatch rather than dividing yourself. NOTE our armor hold time reads LOWER than a KTX end-of-match table on purpose: KTX's clock keeps running after the armor is chewed to zero. efficiency and the shares are RATIOS in [0,1], not percentages. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getBackpacks",
		Description: "RL/LG backpack drops. Returns {backpacks:[...]} where each entry carries time, dropper, weapon ('rl'/'lg'), origin XYZ, resolved loc, a source naming the provenance, and the server ent number. On a 'ktx' row that ent number joins to getWeaponPickups (the grab appears there as source:'backpack' with the same ent); on a 'reconstructed' row it is the backpack entity the linkage bound to the drop and joins to nothing — that row's pickup side is fate/picker on the row itself, below. A 'ktx' row carries one pickup-side field of its own: fate:'expired', set from KTX's third backpack directive //ktx expire, which announces a pack removed UNTAKEN at the 120 s timeout. That is the only wire statement that nobody took a pack, so an ABSENT fate on a 'ktx' row means 'ask getWeaponPickups', never 'nobody took it' — across the archive only half the drops with no pickup row carry the expiry hint, the rest being packs the recording ended on top of. PROVENANCE (source, never mixed within a demo): 'ktx' — decoded from KTX's //ktx drop hint, exact, one per real pack, emitted only by KTX >= 1.38 (49% of the archive); 'reconstructed' — replayed from KTX's own DropBackpack rule (the victim's wielded weapon at the death instant, dropped at their last broadcast position) on older demos, measured at 99.97% precision and recall against the hints. A reconstructed row carries its PICKUP side on the row itself instead of joining to getWeaponPickups: fate ('picked' / 'expired' / 'unobserved') with picker, pickerTeam and pickupTime, read off the wire's backpack-ENTITY track (the pack that appeared at the drop, followed to where it fell, and its disappearance classified by whether a live player's bounding box overlapped it — the same test the server runs before BackpackTouch). Measured against the //ktx bp and //ktx expire hints on 223 demos with every hint withheld: 100% precision and 96.1% recall on picked-vs-not, 99.98% of named pickers correct, and 100% precision and recall on 'expired'. Read 'unobserved' as 'the wire did not answer', NOT as 'nobody took it' — the largest cause is a pack grabbed inside the demo frame it dropped in, which never reaches the wire at all (2% of drops). A 'picked' row with no picker means two players were on the pack and nothing separated them. Still absent on a reconstructed row: kill credit and pack-transfer credit in getPlayerStats, both of which need the hint's hadBefore, which the entity track cannot see. An ABSENT backpacks section is AMBIGUOUS on an old demo: it means either that no RL/LG pack dropped, or that the pass stood down rather than guess (frozen weapon state, no frag log, or a fairpacks/yawnmode/bloodfest ruleset). The wire shape does not distinguish the two, so do not report absence as a refusal. origin is the pack's own position — the victim's origin less the 24 units KTX drops it by — on both provenances. Division of labour across the item trio: getItems = every world spawner's availability/take timeline; getWeaponPickups = who acquired slot weapons and their effectiveness; getBackpacks = the RL/LG drops that feed backpack grabs. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getPointEffects",
		Description: "Point-effect temp-entity stream (schema v71): every point-anchored visual effect the server broadcast, as columnar parallel arrays t (int32 ms) / ty (raw TE code) / c (count byte) / x/y/z (map units). The response's types legend maps the TE codes present in the DEMO to names (explosion, blood, lightningblood, gunshot, teleport, spike, superspike, tarexplosion, wizspike, knightspike, lavasplash; an unnamed code legends as te<code>) — a types-filtered response keeps the full legend so you can see what else the demo carries. What the interesting ones mean: explosion = the EXACT rocket/grenade detonation point (better than the projectile stream's last tracked position); blood = hitscan damage striking a player (per-volley telemetry — on modern KTX c = pellet hits, but the count-byte packaging varies per server generation, so do NOT read c as a damage magnitude across demos; getDamage is the damage answer); lightningblood = an LG cell connecting; gunshot = hitscan wall puff (miss pattern). This is the wire evidence the damage reconstruction consumes on pre-instrumentation demos. Pass types to keep the payload small (blood usually dominates); a filter matching nothing returns empty columns, while pointEffects:null means the demo carried no point effects at all. Response shape: see mvd-api /v1/demos/{id}/streams/point-effects. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in GetPointEffectsInput) (*mcp.CallToolResult, any, error) {
		out, err := b.GetPointEffects(ctx, in)
		return toolResult(out, err)
	})
