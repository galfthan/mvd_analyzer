...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getFrags",
		Description: "Frag aggregates + full kill log. totalFrags + byPlayer (kills/deaths/byWeapon per player) + byWeapon (kills per weapon) + frags (every kill with t/killer/victim/weapon/isSuicide/isTeamKill). NOTE: unlike getDamage/getAim/getItems, summary defaults FALSE here — the kill log is small (one row per frag) and usually the point; pass summary:true to drop it. Optional players= / weapons= filters narrow both aggregates and log. Empty-log convention: frags is null when dropped by summary=true, [] when included but the filter matched nothing. Use this instead of aggregating getEvents(types:['frag']) yourself. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getChat",
		Description: "All-chat and team-chat messages within an optional time window. Returns {messages:[...]} where each message has t, type ('chat' or 'teamsay'), player, team, message (raw with ezQuake markup), messageClean (markup stripped). Cheaper and shape-cleaner than getEvents(types:['chat']) when you only want chat. Descriptive times are integer milliseconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getStateAt",
		Description: "Point-in-time state per player at a given match-relative time. Carry-forward for change streams; nearest-sample for position; interval membership for held items/powerups. Descriptive times are float seconds; the response echoes this in timeUnit.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getLocTrails",
		Description: "Per-player sequence of loc residences with dwell durations. minDwellMs filters nearest-loc flicker (MCP default 250; pass 0 for raw). Each residence is a resolved loc name by default; pass loc='index' for raw LocTable indices (decode via getLocTable). Descriptive times are float seconds; the response echoes this in timeUnit.",
